...
fn sha256Digest(file_path: [:0]const u8, allocator: std.mem.Allocator) !void {
    ...
    const sha256_writer = sha256.writer();
    ...
    while (true) {
        ...
        try sha256_writer.writeAll(buffer[0..bytes_read]);
        if (bytes_read < MAX_BUFFER_SIZE) break;
    }

    checksum_buffer_stream.reset();
    try std.fmt.fmtSliceHexLower(&sha256.finalResult())
        .format("", .{}, checksum_buffer_stream.writer());

    try stdout.writer().print("{s}  {s}\n", .{ checksum_buffer, file_path });
}
